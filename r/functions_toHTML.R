...
statInput <- function(MASTER_TABLE) {
  ...
  tagList(
    ...
    checkboxInput508("showSEs", label="Show standard errors (for percentages)")
  )
}
...
#         href="https://meps.ahrq.gov/survey_comp/precision_guidelines.shtml"))
  ...
  div(id = 'table-tab', class = 'tab-pane active',
      ...
      tags$div(
        ...
        tags$aside(id = "RSE", "*/-- Corresponding crude percentage for the frequency is considered unreliable, as specified in National Center for Health Statistics Data Presentation Standards for Proportions (available from: https://www.cdc.gov/nchs/data/series/sr_02/sr02_175.pdf), so the frequency is not shown.")
      )
  )
}
...
build_plot <- function(pivot) {
  suppressed_HTML <-
    sprintf(
      'Corresponding crude percentage for the frequency is considered unreliable, as specified in National Center for Health Statistics Data Presentation Standards for Proportions (available from: https://www.cdc.gov/nchs/data/series/sr_02/sr02_175.pdf), so the estimate is not shown.',
...
build_main <- function(pivot = F, include = c("table", "plot", "code")) {
  tagList(
    ...
    div(id = 'hc-info',
        ...
        h3("About the data"),
        tags$p("Summary Health Statistics are tables of descriptive statistics for various health measures
including health status, conditions, health behaviors, activity limitations, health insurance coverage,
and access and utilization of health care. Measures are shown for selected characteristics such as sex,
age, race, ethnicity, family income, and region of the United States. Starting with the 2013 NHIS,
Summary Health Statistics are provided in online tables. For the 1997-2012 NHIS, Summary Health Statistics
were provided in three Vital and Health Statistics Series 10 Reports (one each for adults, children,
and the whole population), and for the 1962-1996 NHIS, similar descriptive statistics were provided 
in Current Estimates reports, which were also Vital and Health Statistics Series 10 Reports."),
